	...
	elseif ($chosen[$key][$keyy] == 1) {
		...
		if ($series[$keyy][1] == "") {
			foreach ($chosen as $keyyy => $value) {
				if (getUnit($maaling[$key][1]) == getUnit($maaling[$keyyy][1])) $chosen[$keyyy][$keyy] = 1;
			}
		}
...
//	echo utf8_encode($names[$maaling[$key][0]][0]);
	...
	foreach ($series as $keyy => $value) {
		...
		if (getUnit($series[$keyy][1]) != getUnit($maaling[$key][1]) && $series[$keyy][0] != 0 && $series[$keyy][0] != 0 && $series[$keyy][1] != "") echo " disabled";
		echo ">";
		echo "</td>";
	}
	echo "<td>";
