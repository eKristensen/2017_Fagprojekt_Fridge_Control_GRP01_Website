...
$toggle = isset($_COOKIE['FC_Data_Plot_toggle']) ? json_decode($_COOKIE['FC_Data_Plot_toggle']) : 0;    // om det 24 timer eller alle data der hentes.
$legend = isset($_COOKIE['FC_Data_Plot_legend']) ? json_decode($_COOKIE['FC_Data_Plot_legend']) : 1;    // om det 24 timer eller alle data der hentes.

if (isset($_GET["reset"])) {
	setcookie('FC_Data_Plot_series', '', time()-3600);
	setcookie('FC_Data_Plot_maaling', '', time()-3600);
	setcookie('FC_Data_Plot_chosen', '', time()-3600);
	setcookie('FC_Data_Plot_toggle', '', time()-3600);
	setcookie('FC_Data_Plot_legend', '', time()-3600);
	exit();
}
...
if (isset($_POST["selectgroup"])) {
	...
	$new = array(preg_replace("/[^0-9]+/", "", $value),preg_replace("/[^a-z]+/", "", $value));
	if (!in_array($new, $maaling)) {
		...
		$chosen[] = $fill;
		//$chosen[] = array_fill(0,sizeof($series),0);
	}
	}
...
foreach ($series as $key => $value) {
	echo "<td>";
	if ($value[0] == 0) echo "Måling";
	elseif ($value[0] == 1) echo "Signal";
	elseif ($value[0] == 2) echo "Gennemsnit";
	elseif ($value[0] == 3) echo "Sum";
	if ($value[2] != "") echo "<br>Serienavn:<br>".$value[2];
...
foreach ($maaling as $key => $value) {
	echo "<tr>";
	echo "<td>";
	echo utf8_encode($names[$maaling[$key][0]][1]) . " ";
	echo $maaling[$key][1] . " ";
	...
	foreach ($series as $keyy => $value) {
		...
		if ($series[$keyy][1] != $maaling[$key][1] && $series[$keyy][0] != 0 && $series[$keyy][0] != 0 && $series[$keyy][0] != 1 && $series[$keyy][0] != 1 && $series[$keyy][1] != "") echo " disabled";
		echo ">";
		echo "</td>";
	}
	echo "<td>";
	echo "<button type=\"button\" class=\"btn btn-danger\" onclick=\"remove('maaling','".$key."');\">Fjern</button>";
	echo "</td>";
	echo "</tr>";
}
echo "<tr>";
...
foreach ($series as $key => $value) {
	echo "<td>";
	if ($value[0] != 0) echo "<button type=\"button\" class=\"btn btn-danger\" onclick=\"remove('series','".$key."');\">Fjern</button>";
	echo "</td>";
}
echo "<td><button type=\"button\" class=\"btn btn-success\" onclick=\"getplot();\"";
if ($n == 0) echo " disabled";
echo ">Lav plot</button></td>";
echo "</tr>";
echo "</table>";
...
setcookie('FC_Data_Plot_series', json_encode($series), time()+ (10 * 365 * 24 * 60 * 60));
setcookie('FC_Data_Plot_maaling', json_encode($maaling), time()+ (10 * 365 * 24 * 60 * 60));
setcookie('FC_Data_Plot_chosen', json_encode($chosen), time()+ (10 * 365 * 24 * 60 * 60));
setcookie('FC_Data_Plot_toggle', $toggle, time()+ (10 * 365 * 24 * 60 * 60));
setcookie('FC_Data_Plot_legend', $legend, time()+ (10 * 365 * 24 * 60 * 60));
