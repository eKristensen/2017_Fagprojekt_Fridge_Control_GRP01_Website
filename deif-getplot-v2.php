...
foreach ($akser as $key => $value) {
	...
	foreach ($chosen as $x => $value) {
		foreach ($chosen[$x] as $y => $value) {
			...
			if ($chosen[$x][$y] == 1 && $akser[$key][0] == $series[$y][0]) {
				if ($akser[$key][0] == 2 || $akser[$key][0] == 3) {
					$devices[] = $maaling[$x][0];
					$kinds[] = $maaling[$x][1];
					$note = $series[$y][2];
				} else {
					if (!in_array($akser[$key][0].$maaling[$x][0].$maaling[$x][1], $mesadded)) {

						$mesadded[] = $akser[$key][0].$maaling[$x][0].$maaling[$x][1];
						if ($started) {
							echo ",";
						} else {
							$started = true;
						}
						echo "[".$akser[$key][0].",".json_encode($maaling[$x][0]).",".json_encode($maaling[$x][1]).",";
	...
	if ($akser[$key][0] == 2 || $akser[$key][0] == 3) {
		...
		echo "[".$akser[$key][0].",".json_encode(($devices)).",".json_encode($kinds).",";
